...
function safeParseSkuLS() {
  try {
    const raw = localStorage.getItem("sku");
    if (!raw) return null;
...
export default function GetAQuote() {
  ...
  const onPickProduct = useCallback(
    (opt) => {
      ...
      const p = allProducts.find((x) => x.id === opt.value);
      const thicks = [];
      if (p?.finishes && Array.isArray(p.finishes)) {
        p.finishes.forEach((f) => {
          const exists = thicks.some((t) => t.value === f.thickness_id);
          if (!exists) {
            thicks.push({ value: f.thickness_id, label: f.thickness });
          }
        });
      }
