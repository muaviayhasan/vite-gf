...
class SingleProductComponent extends Component{
    ...
    lightbox() {
        ...
        const bigImages = this.state.images;

        return (
            isOpen && (
                <Lightbox
                    mainSrc={ bigImages[phIndex].path }
                    nextSrc={ bigImages[(phIndex + 1) % bigImages.length].path }
                    prevSrc={ bigImages[(phIndex + bigImages.length - 1) % bigImages.length].path }
