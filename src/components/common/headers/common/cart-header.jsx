...
const CartHeader  = ({item, total, symbol, removeFromCart}) => (
            <li >
                <div className="media">
                    <Link to={`${import.meta.env.VITE_PUBLIC_URL}/product/${item.id}`}><img alt="" className="mr-3" src={`${item.pictures[0]}`} /></Link>
                    <div className="media-body">
                        <Link to={`${import.meta.env.VITE_PUBLIC_URL}/product/${item.id}`}><h4>{item.name}</h4></Link>
