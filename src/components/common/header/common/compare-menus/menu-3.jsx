...
class CompareMenuThree extends Component {
    render() {
                    ...
                    <ul className="compare-products">
                        { compareList.map((item, index) => (
                            <li className="compare-product" key={ index }>
                                ...
                                <h4 className="compare-product-title"><Link  to={ `${import.meta.env.VITE_PUBLIC_URL}/product/default/27`} dangerouslySetInnerHTML={ safeContent(item.name) }></Link></h4>
                            </li>
                        )) }
                        {
...
export default connect( mapStateToProps, { removeFromCompare, resetCompare } )( CompareMenuThree );