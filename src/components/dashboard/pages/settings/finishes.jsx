  ...
  componentDidMount() {
    axios.get(`${import.meta.env.VITE_API_URL}/materials`).then(material => {
      axios.get(`${import.meta.env.VITE_API_URL}/material_types`).then(material_type => {
        axios.get(`${import.meta.env.VITE_API_URL}/finishes`).then(finishes => {
          this.setState({
            ...
            <div className="col-md-9" style={{ "margin-top": "20px" }}>
              <main className="page-content">
                <div className="container-fluid">
                  <h1 className="heading-settings-table">Finishes</h1>
                  {isLoaded ? (
                    <FormTamplate
                      fields={fields}
                      title="Finishes"
                      columns={columns}
                      data={data}
                      saveUrl={`${import.meta.env.VITE_API_URL}/finishes`}
                      updateUrl={`${import.meta.env.VITE_API_URL}/finishes/update`}
                      deleteUrl={`${import.meta.env.VITE_API_URL}/finishes/delete`}
                    />
                  ) : (
                    ""
                  )}
