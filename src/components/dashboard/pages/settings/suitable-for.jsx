...
class SuitableFor extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoaded: false,
  ...
  componentDidMount() {
    axios.get(`${import.meta.env.VITE_API_URL}/suitable_for`).then(res => {
      this.setState({
  ...
  render() {
    ...
    let columns = [{ title: "Suitable For", field: "suitable_for" }];
    return (
      <div>
            ...
            <div className="col-md-9" style={{ "margin-top": "20px" }}>
              <main className="page-content">
                <div className="container-fluid">
                  <h1 className="heading-settings-table">Suitable For</h1>
                  {isLoaded ? (
                    <FormTamplate
                      fields={fields}
                      title="Suitable For"
                      columns={columns}
                      data={data}
                      saveUrl={`${import.meta.env.VITE_API_URL}/suitable_for`}
                      updateUrl={`${import.meta.env.VITE_API_URL}/suitable_for/update`}
                      deleteUrl={`${import.meta.env.VITE_API_URL}/suitable_for/delete`}
                    />
                  ) : (
                    ""
                  )}
