  ...
  componentDidMount() {
    axios.get(`${import.meta.env.VITE_API_URL}/materials`).then(material => {
      axios.get(`${import.meta.env.VITE_API_URL}/material_types`).then(material_type => {
        axios.get(`${import.meta.env.VITE_API_URL}/tile_sizes`).then(tile_sizes => {
          this.setState({
            ...
            <div className="col-md-9" style={{ "margin-top": "20px" }}>
              <main className="page-content">
                <div className="container-fluid">
                  <h1 className="heading-settings-table">Tile Size</h1>
                  {isLoaded ? (
                    <FormTamplate
                      fields={fields}
                      title="Tile Size"
                      columns={columns}
                      data={data}
                      saveUrl={`${import.meta.env.VITE_API_URL}/tile_sizes`}
                      updateUrl={`${import.meta.env.VITE_API_URL}/tile_sizes/update`}
                      deleteUrl={`${import.meta.env.VITE_API_URL}/tile_sizes/delete`}
                    />
                  ) : (
                    ""
                  )}
